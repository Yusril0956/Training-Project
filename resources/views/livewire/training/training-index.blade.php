    ...
    <div class="row row-cards">
        @forelse($trainings as $training)
            @php
                ...
                $icons = [
                    'GK' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-school"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M22 9l-10 -4l-10 4l10 4l10 -4v6" /><path d="M6 10.6v5.4a6 3 0 0 0 12 0v-5.4" /></svg>',
                    ...
                    <div class="card-header">
                        <div class="d-flex align-items-center w-100">
                            ...
                            <div class="flex-grow-1">
                                <h4 class="card-title text-truncate mb-0">{{ Str::limit($training->name, 30) }}</h4>
                    ...
                    <div class="card-footer">
                        @auth
                            @if ($statusBadge === 'accept' || $statusBadge === 'graduate' || Auth::user()->hasAnyRole(['Admin', 'Super Admin']))
                                <a href="{{ route('training.home', $training->id) }}" class="btn btn-primary w-100">Lihat Detail</a>
                            ...
                            @else
                                <form action="{{ route('training.self.register', $training->id) }}" method="POST" class="w-100">
                                    @csrf
                                    <button type="submit" class="btn btn-success w-100">Daftar Pelatihan</button>
                                </form>
                            @endif
                        @else
