        ...
        <div class="card shadow-sm">
            <div class="card-header">
                ...
                <ul class="nav nav-tabs card-header-tabs nav-tabs-alt" id="task-tabs" role="tablist"> 
                    ...
                    @if (Auth::user()->hasAnyRole(['Admin', 'Super Admin']))
                        ...
                        <li class="nav-item">
                            <button class="nav-link @if($defaultTabId === 'tab-admin') active @endif" 
                                id="admin-tab" 
                                data-bs-toggle="tab" 
                    ...
                    @else
                        ...
                        @if (!$userSubmission)
                            <li class="nav-item">
                                <button class="nav-link @if($defaultTabId === 'tab-submit') active @endif" 
                                    id="submit-tab" 
                                    data-bs-toggle="tab" 
                                    data-bs-target="#tab-submit"
                                    type="button" 
                                    role="tab" 
                        ...
                        @if ($userSubmission)
                            <li class="nav-item">
                                <button class="nav-link @if($defaultTabId === 'tab-view') active @endif" 
                                    id="view-tab" 
                                    data-bs-toggle="tab" 
                        ...
                        @if ($userSubmission && $task->deadline->isFuture() && !$userSubmission->review)
                            <li class="nav-item">
                                <button class="nav-link @if($defaultTabId === 'tab-edit') active @endif" 
                                    id="edit-tab" 
                                    data-bs-toggle="tab" 
            ...
            <div class="card-body">
                <div class="tab-content" id="task-tabs-content">
                    ...
                    @if (Auth::user()->hasAnyRole(['Admin', 'Super Admin']))
                        <div class="tab-pane fade @if($defaultTabId === 'tab-admin') show active @endif" 
                             id="tab-admin" 
                             role="tabpanel" 
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('admin.submission.download', $submission->id) }}"
                                                            class="btn btn-icon btn-sm btn-primary" title="Download">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0 0z" fill="none"/><path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2 -2v-2" /><path d="M7 11l5 5l5 -5" /><path d="M12 4v12" /></svg>
                                                        </a>
                                                        <a href="{{ route('admin.task.review', [$training->id, $task->id, $submission->id]) }}"
                                                            class="btn btn-icon btn-sm btn-success" title="Nilai">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0 0z" fill="none"/><path d="M9 7h-3a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-3" /><path d="M9 15h3l8.5 -8.5a1.5 1.5 0 0 0 -3 -3l-8.5 8.5v3" /><line x1="16" y1="5" x2="19" y2="8" /></svg>
                                                        </a>
                                                    </td>
                                                </tr>
                                            @endforeach
                    ...
                    @else
                        ...
                        @if (!$userSubmission)
                            <div class="tab-pane fade @if($defaultTabId === 'tab-submit') show active @endif" 
                                ...
                                @if ($task->deadline->isPast())
                                    <div class="alert alert-danger">❌ Deadline tugas telah terlewat. Anda tidak bisa mengirim tugas.</div>
                                @else
                                    <form wire:submit.prevent="submitTask" enctype="multipart/form-data" 
                                          x-data="{ fileInput: null }" 
                                          x-init="fileInput = $refs.submissionFile">
                                        <div class="mb-3">
                                            <label class="form-label">File Tugas <span class="text-danger">*</span></label>
                                            <input type="file" wire:model.live="submission_file" x-ref="submissionFile"
                        ...
                        @if ($userSubmission)
                            <div class="tab-pane fade @if($defaultTabId === 'tab-view') show active @endif" 
                                ...
                                <p class="text-muted"><strong>Dikirim pada:</strong> {{ $userSubmission->submitted_at->format('d M Y H:i') }}</p>
                                ...
                                <a href="{{ asset('storage/' . $userSubmission->file_path) }}" target="_blank"
                                    class="btn btn-sm btn-outline-secondary mb-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon me-1" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0 0z" fill="none"/><path d="M10 12l-4 4l-4 -4" /><path d="M6 16v-8a4 4 0 0 1 4 -4h4" /><path d="M14 10h6v6a2 2 0 0 1 -2 2h-6l4 -4l-4 -4" /></svg>
                                ...
                                @if ($fileIsImage)
                                    ...
                                    <div class="mt-2 mb-4">
                                        <img src="{{ asset('storage/' . $userSubmission->file_path) }}" alt="Pratinjau Kiriman"
                                ...
                                @if ($task->deadline->isPast() && !$userSubmission->review)
                                    <div class="alert alert-warning">⚠️ Tugas sudah melewati deadline. Menunggu penilaian dari pengajar.</div>
                                @elseif ($userSubmission->review)
                                    <div class="alert alert-info"> Tugas sudah dinilai. Silakan cek tab **Lihat Penilaian**.</div>
                                @endif
                            </div>
                        @endif
                        ...
                        @if ($userSubmission && $task->deadline->isFuture() && !$userSubmission->review)
                            <div class="tab-pane fade @if($defaultTabId === 'tab-edit') show active @endif" 
                                ...
                                <form wire:submit.prevent="editTask" enctype="multipart/form-data"
                                     x-data="{ fileInput: null }" 
                                     x-init="fileInput = $refs.submissionFile; $wire.on('show-tab', ({ tabId }) => { 
                                         if (tabId === 'tab-view') {
                                    ...
                                    <div class="alert alert-info">Anda dapat mengganti file dan/atau pesan. File saat ini: **{{ basename($userSubmission->file_path) }}**</div>
                                    <div class="mb-2">
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <button type="button" class="btn btn-outline-secondary" data-bs-toggle="tab" data-bs-target="#tab-view">
                                            Batal
                                        </button>
                                    </div>
                                    <div x-init="$wire.on('fileSubmitted', () => { fileInput.value = ''; })"></div>
                        ...
                        @if ($userSubmission && $userSubmission->review)
                            <div class="tab-pane fade @if($defaultTabId === 'tab-review') show active @endif" 
                                ...
                                <div class="card card-body bg-light-lt">
                                    <p class="h4">Nilai: <span class="badge bg-green-lt text-green-lt-fg">{{ $userSubmission->review->score }}</span></p>
                                    <p><strong>Komentar:</strong> {{ $userSubmission->review->comment ?? '-' }}</p>
                                    <p class="text-muted small">
                                        Dinilai oleh: {{ $userSubmission->review->reviewer->name }} pada {{ $userSubmission->review->created_at->format('d M Y') }}
                                    </p>
                                </div>
                                {{-- Gunakan data-bs-target untuk kembali ke view --}}
                                <button type="button" class="btn btn-outline-secondary mt-3" data-bs-toggle="tab" data-bs-target="#tab-view">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="icon me-1" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0 0z" fill="none"/><path d="M14 3v4a1 1 0 0 0 1 1h4" /><path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" /><path d="M9 17h6" /><path d="M9 13h6" /></svg>
                                    Lihat Kiriman
                                </button>
                            </div>
                        @endif
...
<script>
    document.addEventListener('livewire:navigated', () => {
        ...
        const showDefaultTab = () => {
            ...
            const tabElement = document.querySelector(`#task-tabs button[data-bs-target="#${defaultTabId}"]`);
            ...
            if (tabElement && tabContentElement) {
                ...
                document.querySelectorAll('#task-tabs .nav-link').forEach(btn => btn.classList.remove('active'));
                document.querySelectorAll('#task-tabs-content .tab-pane').forEach(pane => pane.classList.remove('show', 'active'));
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const tabElement = document.querySelector(`#task-tabs button[data-bs-target="#${initialDefaultTabId}"]`);
